        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function fetchAvailableSheets() {
                try {
                    ...
                    if (data.sheets && data.sheets.length > 0) {
                        availableSheets = data.sheets.map(sheet => sheet.properties.title);
            ...
            function populateSheetDropdown(sheets) {
                ...
                const sortedSheets = sheets.sort().reverse();
                ...
                // REMOVED: Auto-select the first sheet - now user must manually select
            }
            ...
            async function performLookup() {
                ...
                showStatus(`Searching  UTR- ${selectedSheet}...`, 'progress');
                lookupBtn.disabled = true;
                
                try {
                    const result = await searchUTRInSheet(selectedSheet, extractedUTR);
                    displayResult(selectedSheet, inputValue, extractedUTR, result);
                    addToHistory(selectedSheet, inputValue, extractedUTR, result);
            ...
            async function searchUTRInSheet(sheetName, utrValue) {
